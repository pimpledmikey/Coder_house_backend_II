...
const getAll = async () => {
  const carts = await cartModel.find(); // Buscamos todos los carritos en la base de datos de mongo
  return carts;
};

const getById = async (id) => {
  const cart = await cartModel.findById(id).populate("products.product"); // Buscamos un carrito por id en la base de datos de mongo y poblamos el campo products con la información de los productos
  return cart;
};
